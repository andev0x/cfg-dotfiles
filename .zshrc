...
alias clean-hzsh='cp ~/.zsh_history ~/.zsh_history.bak && awk -F ";" "!seen[\$2]++" ~/.zsh_history > ~/.zsh_history_clean && mv ~/.zsh_history_clean ~/.zsh_history && fc -R ~/.zsh_history'
...
#setopt SHARE_HISTORY            # disable if using Atuin (avoid dupes)
setopt HIST_EXPIRE_DUPS_FIRST    # expire oldest duplicates first
setopt HIST_IGNORE_ALL_DUPS      # don’t store duplicates at all
setopt HIST_SAVE_NO_DUPS         # don’t save dupes to file
setopt HIST_FIND_NO_DUPS         # skip duplicates in reverse search (Ctrl+R)
setopt HIST_REDUCE_BLANKS        # trim extra blanks
setopt HIST_VERIFY               # confirm before reusing from history
